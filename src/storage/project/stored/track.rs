...
impl StoredTrack {
    ...
    pub fn from_track(track: &dyn Track) -> Option<Self> {
...
pub(crate) struct StoredAudioTrack {
    ...
    pub regions: HashMap<RegionID, AudioRegion>,
}

impl StoredAudioTrack {
    pub fn from_audio_track(track: &AudioTrack) -> Self {
        Self {
...
pub(crate) struct StoredNoteTrack {
    pub graph: StoredGraph,
    pub regions: HashMap<RegionID, NoteRegion>,
}

impl StoredNoteTrack {
    pub fn from_note_track(track: &NoteTrack) -> Self {
        Self {
            graph: StoredGraph::from_graph(track.get_graph()),
            regions: track.get_all_regions().clone(),
        }
