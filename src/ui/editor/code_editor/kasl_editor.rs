...
pub(super) fn kasl_editor(ui: &mut egui::Ui, state: &mut EditorState, panel_id: egui::Id) {
    ...
    let (Some(theme), Some(syntect_settings)) = (
        state.ui_state.code_editor_state.theme.as_ref(),
        state.ui_state.code_editor_state.syntect_settings.as_ref(),
    ) else {
        return;
    };
