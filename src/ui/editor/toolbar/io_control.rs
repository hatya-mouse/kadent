            ...
            .show(|ui| {
                ...
                let mut audio_output_item =
                    |ui: &mut egui::Ui, device_name: String, device: &cpal::Device| {
                        ...
                        let is_selected = state.ui_state.selected_output_device == device_id;

                        if ui.selectable_label(is_selected, &device_name).clicked() && !is_selected
                        {
                            // Select the label when clicked
                            state.ui_state.selected_output_device = device_id;
                            state
                                .thread_handle
                                .audio_command_tx
                ...
                .ui(ui, |ui| {
                    if let Some(default_output_device) = &state.ui_state.default_output_device {
                ...
                .ui(ui, |ui| {
                    ...
                    let Some(midi_in) = &state.ui_state.midi_in else {
                        return;
                    };
                    ...
                    if ui
                        .selectable_label(
                            state.ui_state.selected_midi_port.is_none(),
                            "No MIDI Input",
                        )
                        .clicked()
                        && state.ui_state.selected_midi_port.is_some()
                    {
                        state.push_action(EditorAction::DisconnectMidiPort);
                    }

                    for (ref name, midi_in_port) in midi_in_port_names {
                        let is_selected =
                            state.ui_state.selected_midi_port.as_ref() == Some(&midi_in_port.id());
                        if ui.selectable_label(is_selected, name).clicked() && !is_selected {
                            // Set the selected MIDI input port
                            state.push_action(EditorAction::SetMidiInputPort(midi_in_port));
                        }
                    }
                });
