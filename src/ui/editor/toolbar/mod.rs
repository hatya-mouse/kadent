...
pub(super) fn toolbar(ui: &mut egui::Ui, state: &mut EditorState) {
    ui.horizontal_centered(|ui| {
        ...
        vu_meter(
            ui,
            &state.ui_state.toolbar_state.last_vu_value,
            &state.ui_state.toolbar_state.peak_holds,
...
fn playhead_beats(ui: &mut egui::Ui, state: &EditorState) {
    let playhead_beats =
        state.ui_state.playhead_tick.0 as f32 / state.ui_state.audio_ctx.resolution as f32;
    toolbar_group(ui, |ui| {
        ui.add_sized(
            [200.0, 28.0],
            egui::Label::new(
