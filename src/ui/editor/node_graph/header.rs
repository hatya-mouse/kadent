...
pub(super) fn draw_node_graph_header(ui: &mut egui::Ui, state: &mut EditorState) {
    ...
    if jump_to_random
        && let Some(track_id) = state.ui_state.selection.track_id()
    ...
    {
        state.ui_state.node_graph_state.jump_to_pos = Some(node_meta.pos);
    }
    ...
    if let Some(node_type) = node_to_add {
        ...
        let pan = state.ui_state.node_graph_state.pan_offset;
        let pos = egui::pos2(-pan.x + 20.0, -pan.y + 20.0);
        state.push_action(EditorAction::AddNode(track_id, node_type, pos));
    }
}
