...
pub(super) fn status_bar(ui: &mut egui::Ui, state: &mut EditorState) {
    ui.horizontal_centered(|ui| {
        if let Some(track_id) = state.ui_state.selection.track_id()
            && let Some(track_meta) = state.ui_state.proj_ctx.project_meta.get_track(&track_id)
        {
            ...
            if let Some(region_id) = state.ui_state.selection.region_id()
            ...
            if let Some(node_id) = state.ui_state.selection.node_id()
        ...
        if let Some(task) = &state.ui_state.status_bar_state.current_task {
            status_text(
                ui,
                match task {
        ...
        modification_text(state, ui);

        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            state.ui_state.status_bar_state.notification_text(ui);
        });
    });
}

fn modification_text(state: &EditorState, ui: &mut egui::Ui) {
    if state.ui_state.modification.is_none() {
        return;
    }

    let resolution = state.ui_state.audio_ctx.resolution as f32;
    let modification_string = match state.ui_state.modification {
