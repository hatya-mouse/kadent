    ...
    ui::{
        ...
        editor::{
            ...
            timeline::{
                ...
                edit_panel::{
                    region_notes::draw_notes_in, waveform::draw_waveform_in, x_to_ticks,
                    y_to_track_id,
                },
            },
        },
...
fn draw_regions(
    ...
    let Some(track_meta) = state.ui_state.proj_ctx.project_meta.get_track(track_id) else {
        return;
    };
    ...
    for region_id in region_ids {
        ...
        let new_region_x = row_rect.min.x + TIMELINE_LEFT_PADDING + current_start.0 as f32 * ppt;
        ...
        let stroke =
            if state.ui_state.selection.track_and_region_id() == Some((*track_id, region_id)) {
        ...
        if track_meta.track_type == TrackType::Audio {
            draw_waveform_in(ui, &state.ui_state, *track_id, region_id, &draw_rect);
        } else {
            draw_notes_in(ui, &state.ui_state, track_id, &region_id, &draw_rect);
        }
    }
}
    ...
    if response.double_clicked() {
        ...
        match track_type {
            Some(TrackType::Audio) => {
                state.push_action(EditorAction::AddAudioRegion(
                    *track_id,
                    "Region".to_string(),
                    bounds,
                ));
            ...
            Some(TrackType::Note) => {
                state.push_action(EditorAction::AddNoteRegion(
                    *track_id,
                    "Region".to_string(),
                    bounds,
                ));
...
fn region_gestures(
    ...
    if resize_res.dragged() {
            ...
            .and_then(|track| track.get_region_mut(region_id))
        {
            let (region_start, region_end) = region
                .bounds
                .tick_range(&state.ui_state.proj_ctx.project.tempo_map);
            let region_duration = region_end - region_start;
    ...
    {
        state.push_action(EditorAction::SetRegionDuration(
            *track_id,
            *region_id,
    ...
    if move_res.dragged() {
        ...
        && let Some(new_start) = state
            ...
            .map(|region| {
                region
                    .bounds
                    .start_tick(&state.ui_state.proj_ctx.project.tempo_map)
            })
    {
