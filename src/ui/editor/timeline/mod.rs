...
impl KnodiqApp {
    pub(in crate::ui) fn timeline(&mut self, ui: &mut egui::Ui) {
        ...
        let track_list_width = self.ui_state.editor_state.timeline_state.track_list_width;
        let timeline_scroll_y = self.ui_state.editor_state.timeline_state.timeline_scroll_y;

        // Get the scroll delta
        let scroll_delta = ui.input(|i| i.smooth_scroll_delta.y);
        self.ui_state.editor_state.timeline_state.timeline_scroll_y -= scroll_delta;
        ...
        self.ui_state.editor_state.timeline_state.timeline_scroll_y =
            timeline_scroll_y.clamp(0.0, max_scroll);

        // Synchronize the scroll amount
        let scroll_y = timeline_scroll_y;
        ...
        if divider_resp.dragged() {
            self.ui_state.editor_state.timeline_state.track_list_width +=
