...
impl EditorUi {
    pub(crate) fn calculate_playhead(&mut self) {
        self.state.transport.playhead_tick = Ticks(
            self.state
                .thread_handle
                .playhead_tick
    ...
    pub(crate) fn process_vu_value(&mut self) {
        let channels = self.state.project.meta.export_ctx.channels;
        ...
        for channel in 0..channels {
            ...
            if let Some(v) = self.state.thread_handle.vu_consumer.try_pop() {
