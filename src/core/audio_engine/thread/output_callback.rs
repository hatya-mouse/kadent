...
pub(super) fn output_callback(
    ...
    let build_result = device.build_output_stream(
        ...
        move |data: &mut [f32], _| {
            ...
            while let Some(command) = ctx.command_cons.try_pop() {
                match command {
                    AudioCommand::Seek(seek_position) => {
                        let playhead_sample = seek_position
                            .in_sample(&mixer.project.tempo_map, mixer.playback_ctx.sample_rate);
            ...
            mixer.process(false, is_playing, current_playhead as usize, data);
    ...
    match build_result {
        ...
        Err(err) => {
            if let Ok(ctx) = result_ctx.lock() {
                let _ = ctx.result_tx.send(Err(AudioError::BuildStreamError(err)));
            }
            None
        }
    }
